...
const VirtualGallery = () => {
    const canvasRef = useRef(null);
    const infoCardRef = useRef(null);
    const buyBtnRef = useRef(null); // Ref for the Buy Now button
        ...
        const updatePaintingInfo = () => {
            ...
            if (paintingToShow && infoCardRef.current) {
                ...
                infoCardRef.current.innerHTML = `
                    ...
                    <h2>${category}</h2>
                    <p>${description}</p>
                    <button ref={buyBtnRef} id="buyBtn" class="buyBtn">Buy Now</button>
                `;
                infoCardRef.current.dataset.paintingId = _id;
                infoCardRef.current.style.display = 'block';
                
                setPaintingId(_id); // Update painting ID for Buy Now button
            } else if (infoCardRef.current) {
                infoCardRef.current.innerHTML = '';
                infoCardRef.current.dataset.paintingId = '';
            }
    ...
    // Use effect to handle the button click event
    useEffect(() => {
        if (paintingId && buyBtnRef.current) {
            buyBtnRef.current.onclick = () => {
                const api = 'https://pixelated-canvas.onrender.com';
    ...
    return (
        <div style={{ position: 'relative' }}>
            <canvas ref={canvasRef} />
            <div ref={infoCardRef} className="infoCard" />
        </div>
    );
};
